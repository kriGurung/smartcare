...
export default function AdminPayments() {
  ...
  async function refund(p) {
    if (!window.confirm(`Refund ${formatNpr(p.amountPaisa)} to ${p.booking?.patientName || 'the patient'}?`)) return;
    setBusyId(p.id);
    setError('');
    try {
            ...
            {payments.map((p) => (
              <div key={p.id} className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex items-center gap-3">
                  ...
                  {p.status === 'paid' && (
                    <Button size="sm" variant="outline" icon={RotateCcw} loading={busyId === p.id} onClick={() => refund(p)}>Refund</Button>
                  )}
                </div>
              </div>
            ))}
