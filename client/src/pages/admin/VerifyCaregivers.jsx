...
export default function VerifyCaregivers() {
  ...
  return (
    <div>
                    ...
                    return (
                      <div key={d.id} className="rounded-xl border border-slate-100 p-3">
                        ...
                        <div className="mt-3 flex gap-2">
                          ...
                          <Button size="sm" variant="outline" icon={X} onClick={() => setConfirmDoc(d.id)} className="flex-1">Reject</Button>
                        </div>
                      </div>
                    );
                  })}
      ...
      {/* Per-document reject confirm */}
      {confirmDoc && (
        <ConfirmDialog
          open
          title="Reject this document?"
          message="The document will be marked as rejected and the caregiver will be notified to upload a corrected copy."
          confirmLabel="Yes, reject document"
          onConfirm={() => {
            const docId = confirmDoc;
            setConfirmDoc(null);
            reviewDoc(docId, 'rejected');
          }}
