    ...
    <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {u.role !== 'admin' && (
                    <Button
                      ...
                      loading={busyId === u.id}
                      onClick={() => (u.status === 'suspended' ? toggleStatus(u) : setConfirmUser(u))}
                    >
      ...
      {confirmUser && (
        <ConfirmDialog
          ...
          message={
            <>
              <span className="font-semibold text-ink">{confirmUser.name}</span> will no longer be able to log in or
              use SmartCare until an admin reactivates them.
            </>
          }
          confirmLabel="Yes, suspend user"
          loading={busyId === confirmUser.id}
          onConfirm={() => {
            const u = confirmUser;
            setConfirmUser(null);
            toggleStatus(u);
