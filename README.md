...
| `npm test` | Run the automated test-suite (Vitest + Supertest) |
...
**Optional — run the whole stack in containers (from the project root)**

```bash
docker compose up --build
# PWA → http://localhost:8080   API → http://localhost:5000   MySQL → localhost:3307
docker compose exec api node src/utils/seed.js   # load demo data once
```

This mirrors the production architecture (nginx + Express + MySQL) without
installing anything locally. The Dockerfiles live at `server/Dockerfile` and
`client/Dockerfile`; config is in `docker-compose.yml`.

---

## Testing

The backend ships with a Vitest + Supertest suite (unit tests for money
handling, field encryption, JWT and the payment service, plus end-to-end
integration tests for auth, the booking → payment → review lifecycle, and
RBAC / data scoping). Run it from `/server`:

```bash
npm test
```

Tests run against an **in-memory SQLite** database in forked workers, so they
need no MySQL, hit no real gateway, and never touch your local data. The
runtime stack stays exactly per the report: Node.js + Express + MySQL.

---

## Deployment (AWS Free Tier)

Continuous integration and deployment are set up in `.github/workflows`:

| Workflow | When | What it does |
|----------|------|--------------|
| `ci.yml` | Every push & PR | Installs deps, runs the API test suite, builds the PWA, audits prod dependencies |
| `deploy.yml` | Push to `main` | Re-runs the checks, then SSHes to EC2 and runs `deploy/deploy.sh` |

The production target matches the report (§3.3.7): **EC2 t2.micro** for the
API behind nginx, **RDS MySQL**, **S3** for static assets/documents, and
**CloudWatch** for monitoring.

Full step-by-step instructions (RDS, EC2 bootstrap, systemd, nginx, S3,
CloudWatch, and the GitHub secrets to configure) are in
