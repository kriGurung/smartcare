...
// ":memory:" is used by the test-suite; file-based DBs are resolved relative
// to the server root.
const storage = process.env.DB_STORAGE;
export const sequelize = isSqlite
  ? new Sequelize({
      ...common,
      // Single connection: with :memory: storage a second connection would be
      // a brand-new empty database, silently breaking multi-query workflows.
      pool: { max: 1, min: 0, acquire: 30000, idle: 10000 },
      storage: storage === ':memory:' ? ':memory:' : path.resolve(__dirname, '../../', storage || 'smartcare.sqlite'),
    })
