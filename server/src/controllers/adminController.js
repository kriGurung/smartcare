...
// GET /api/admin/reports/top-caregivers — top caregivers by rating
export const getTopCaregivers = asyncHandler(async (req, res) => {
  const caregivers = await User.findAll({
    where: { role: ROLES.CAREGIVER },
    attributes: ['id', 'name'],
    include: [
      {
        ...
        required: true,
        attributes: ['avg_rating', 'total_reviews', 'hourly_rate_paisa', 'city'],
      },
    ],
    order: [[{ model: CaregiverProfile, as: 'caregiverProfile' }, 'avg_rating', 'DESC']],
    limit: 10,
  });

  const caregiverIds = caregivers.map((c) => c.id);
  const bookingCounts = await Booking.findAll({
    where: { caregiver_id: { [Op.in]: caregiverIds }, status: BOOKING_STATUS.COMPLETED },
    attributes: ['caregiver_id', [fn('COUNT', col('id')), 'completed_bookings']],
    group: ['caregiver_id'],
    raw: true,
  });
  const countMap = new Map(bookingCounts.map((b) => [b.caregiver_id, Number(b.completed_bookings)]));

  const top = caregivers.map((c) => ({
    id: c.id,
    name: c.name,
    city: c.caregiverProfile?.city || null,
    avgRating: Number(c.caregiverProfile?.avg_rating || 0),
    totalReviews: c.caregiverProfile?.total_reviews || 0,
    hourlyRatePaisa: c.caregiverProfile?.hourly_rate_paisa || 0,
    completedBookings: countMap.get(c.id) || 0,
  }));

  res.json({ topCaregivers: top });
});
